...
import 'filefolder_info.dart';

class FileListPage extends StatefulWidget {
  final String rootPath;
  const FileListPage({super.key, required this.rootPath});
...
class _FileListPageState extends State<FileListPage> {
  ...
  bool _newFolderNameValidate = false;
  ...
  void _updateBreadcrumbs() {
    ...
    widget.rootPath == "/storage/emulated/0"
        ? breadcrumbs.add(BreadCrumbItem(
            content: const Text('Internal'),
            onTap: () {
              setState(() {
    ...
    if (widget.rootPath == "/storage/emulated/0") {
      subPaths = _currentDirectory!.path.split('/').sublist(4);
    } else {
  ...
  void createFolder() {
    setState(() {
      if (newFolderNameController.text.isEmpty) {
        _newFolderNameValidate = true;
      } else {
        _newFolderNameValidate = false;
        String newFolderPath = '';

        newFolderPath =
            '${_currentDirectory!.path}/${newFolderNameController.text}';
        if (Directory(newFolderPath).existsSync()) {
          showDialog(
            context: context,
            builder: (context) {
              return AlertDialog(
                content: Text("${newFolderNameController.text} already exists"),
                actions: [
                  TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: widget.rootPath == "/storage/emulated/0"
              ? const Text("Internal")
              : const Text("SD Card"),
          actions: [
            IconButton(
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                              errorText: _newFolderNameValidate
